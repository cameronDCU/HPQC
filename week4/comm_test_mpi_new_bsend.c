...
// Function to perform non-root task using MPI_Bsend
void non_root_task(int my_rank, int uni_size) {
    int send_message = my_rank * 10, count = 1, dest = 0, tag = 0;
    int buf_size = MPI_BSEND_OVERHEAD + sizeof(int); // Ensure enough space for buffering
    void *buffer = malloc(buf_size);

    MPI_Buffer_attach(buffer, buf_size);
    MPI_Bsend(&send_message, count, MPI_INT, dest, tag, MPI_COMM_WORLD);
    printf("Hello, I am %d of %d. Buffered Sent %d to %d\n", my_rank, uni_size, send_message, dest);
